const API_KEY = "IFRkcfJOxpoeO7PwFyl-kaEt3bc";
...
function displayException(data) {
  ...
  let exception = `<div>The API returned status code ${data.status_code}</div>`;
  exception += `<div>Error number: <strong>${data.error_no}</strong></div>`;
  exception += `<div>Error text: <stong>${data.error}</strong></div>`;
  document.getElementById("resultsModalTitle").innerText = heading;
